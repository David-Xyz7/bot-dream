//BISMILLAH HIRAHMANINROHIM
//JAN CUMA AMBIL CASE NYA DOANG/NUMPANG AMBIL SC DOANG 
//KASIH TQTO NYA + AUTHOR NYA
//NOTE : BOT BLOM ALL FITUR WORK
//JADI KALO MAU YANG ALL WORK + FITUR 500 TEMBUSIN 1K SUBS
//DI 1K SUBS GWA BAKAL BIKIN SC BOT YANG LEBIH GG
//DAN ALL FITUR WORK
//MAKNYA TEMBUSIN 1K SUBS CHANEL DREAM BOT
//KALO ADA YANG EROR BILANG ASW JAN CUMA BENTAK DOANG NGENTOD
//KALO ADA YANG EROR PC GWA
const sotoy = [
		'🍊 : 🍒 : 🍐',
		'🍒 : 🔔 : 🍊',
		'🍇 : 🍇 : 🍇',
...
const sleep = async (ms) => {
    return new Promise(resolve => setTijmeout(resolve, ms));
}
//***********SETING DULU BANG KALO MAU MAH ASAL JAN HAPUS PARA MASTAH***********//
prefix = ''
f = '*'
blocked = []
namabot = 'DREAM	'
...
const Lolhuman = 'b170074ac846042f35937286' // Beli.Sendiri.tod
//*FUCIONT**//
...
async function starts() {
	...
	fs.existsSync('./Zitsraa.json') && Vid.loadAuthInfo('./Zitsraa.json')
	Vid.on('connecting', () => {
...
sᴇᴍᴏɢᴀ ᴊᴀsᴀᴅᴍᴜ ʙᴀɪᴋ ʙᴀɪᴋ sᴀᴊᴀ ᴅᴀɴ sᴇʟᴀʟᴜ ᴅɪᴋᴇɴᴀɴɢ ᴏʟᴇʜ ᴏʀᴀɴɢ ʏɢ ʙᴇʀᴀᴅᴀ ᴅɪsɪɴɪ🚮`
			...
			let buff = await getBuffer(ppimg)
			teks = `𝙋𝙍𝙊𝙈𝙊𝙏𝙀 𝘿𝙀𝙏𝙀𝘾𝙏
*Selamat @${num.split('@')[0]} Telah Menjadi Admin Di Gc* ${mdata.subject}`
...
break
         ...
         break
                ...
                const serialUser = createSerial(20)
                if(isNaN(umurUser)) return await reply('*Umur harus berupa angka!*')       
                if (namaUser.length >= 30) return reply(`*ITU NAMA ATAU JALAN KERETA PANJANG AMAT*`)
...
case '!upswtext':
				...
				case '!upswvideo':
                 ...
                 reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix}menu  Bot ${namabot}*`)
